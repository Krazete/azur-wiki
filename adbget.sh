...
while IFS=$'\t\r\n' read -r line; do
    if [[ $line == *: ]]; then
        ...
        # mtime=$(adb shell -n stat -c %Y $path) # unused alternative; too expensive and slow
        age=$(($now - $mtime - 7*86400)) # date difference minus a week
        if [[ $age == -* ]]; then
            name=${BASH_REMATCH[2]}
            path=$parent/$name # file path on phone
            dstparent=${parent:48}
            mkdir -p $dstparent # create folder on computer
            dstpath=$dstparent/$name # file path for computer
            adb pull -a $path $dstpath # copy file with mtime preserved
        else
